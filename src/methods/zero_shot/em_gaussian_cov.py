...
class BASE(object):
    ...
    def run_task(self, task_dic):
        """
        inputs:
            task_dic : dictionnary with n_task zero-shot tasks
        """
...
class EM_GAUSSIAN_COV(BASE):
    ...
    def s_init(self, query):
        ...
        """
        d_q = ((self.w.unsqueeze(1) - query.unsqueeze(2)).square_()
               ).mul_(self.u.unsqueeze(3)).sum(1)
    ...
    def s_update(self, query):
        ...
        nonzero_clusters = cluster_sizes > self.eps
        self.s = (self.u.sum(1)).unsqueeze(2) / d_q.clamp(min=self.eps) * \
            nonzero_clusters + (self.s * (1 - 1*nonzero_clusters))
    ...
    def run_method(self, query, y_q):
        ...
        self.logger.info(
            " ==> Executing EM_GAUSSIAN_COV with T = {}".format(self.args.T))
